...
def compute_crc_and_stuff(stuffed_data):
	...
	crc_array = to_bytes(">H",2,crc_int)
...
def compute_pktlen_and_stuff(stuffed_data):
	...
	len_array = to_bytes(">H",2, pkt_length)
...
def compute_channel_bit(active, channel_number, num_pts):
	...
	# return channel_bit.to_bytes(1,'big') # python 3 has to_bytes
	return to_bytes(">B",1,channel_bit)
...
def compute_point(dur, amp):
	...
	# return (bin_dur | bin_amp).to_bytes(4,'big') # Python 3 has to_bytes
	return to_bytes(">I",4,(bin_dur | bin_amp))
...
def generate_packet(pkt_num, command_num, active, channel, point_list):
	...
	command_pkt = to_bytes(">H", 2, ((pkt_num << 10) | (command_num)))
...
def to_bytes(fmt, length, n):
	val = struct.pack(fmt, n) # want big endian
	byte_arr =[i for i in struct.unpack(length*"B", val)]
...
# print (binascii.hexlify(combined_test_pkt))
# print (binascii.hexlify(example_packet))
